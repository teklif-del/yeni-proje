...
function dbToOdeme(row) {
  return {
    ...
    kdvOrani:     Number(row.kdv_orani) || 0,
    kdvTutar:     Number(row.kdv_tutar) || 0,
...
function FaturaModal({ odeme, mulk, acik, kapat, onFaturaKes }) {
  ...
  // Girilen tutar KDV DAHİL → KDV hariç matrahı ve KDV tutarını hesapla
  const kdvDahilToplam = odeme?.tutar || 0;
  const matrah         = kdvOrani > 0 ? Math.round(kdvDahilToplam / (1 + kdvOrani / 100)) : kdvDahilToplam;
  const kdvTutar       = kdvDahilToplam - matrah;

  const kes = () => {
    onFaturaKes(odeme.id, faturaNo, faturaTarihi, kdvOrani, kdvTutar, matrah);
    kapat();
  };
    ...
    <Modal acik={acik} kapat={kapat} baslik="🧾 Fatura Kes" genislik="480px">
      ...
      <div style={{ background:'linear-gradient(135deg,#F0FDF4,#DCFCE7)', borderRadius:'12px', padding:'16px', marginBottom:'14px', border:'1px solid #86EFAC' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px' }}>
          ...
          <InfoSatir etiket="Ödenen Tutar (KDV Dahil)" deger={`₺${kdvDahilToplam.toLocaleString('tr-TR')}`} renk="#15803D" />
      ...
      <div style={{ background:'linear-gradient(135deg,#EFF6FF,#DBEAFE)', borderRadius:'12px', padding:'14px 16px', marginBottom:'14px', border:'1.5px solid #93C5FD' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'8px' }}>
          <div style={{ background:'white', borderRadius:'8px', padding:'10px', border:'1px solid #BFDBFE', textAlign:'center' }}>
            ...
            <div style={{ fontSize:'15px', fontWeight:'800', color:'#1E293B' }}>₺{matrah.toLocaleString('tr-TR')}</div>
          </div>
          <div style={{ background:'white', borderRadius:'8px', padding:'10px', border:'1px solid #BFDBFE', textAlign:'center' }}>
...
//  ANA SAYFA
// ═══════════════════════════════════════════════════════════
const GIDER_KATEGORILERI = ['Bakım/Onarım','Vergi','Sigorta','Yönetim Komisyonu','Elektrik/Su/Doğalgaz','Temizlik','Asansör','Site Aidatı','Tapu/Noter','Diğer'];

function dbToGider(row) {
  return {
    id:        row.id,
    mulkId:    row.mulk_id || null,
    tarih:     row.tarih || '',
    tutar:     Number(row.tutar) || 0,
    kategori:  row.kategori || 'Diğer',
    aciklama:  row.aciklama || '',
  };
}
...
export default function KiraPage() {
  ...
  const netKar           = toplamOdenenKira - toplamGider;

  // Toplam fatura hesapları (kesilen faturalar)
  // matrah = kdv hariç, kdvTutar = hesaplanan kdv, tutar = kdv dahil toplam (girilen rakam)
  const toplamFaturaTutar   = useMemo(() => faturaKesilenOdemeler.reduce((s, o) => s + (o.matrah || o.tutar), 0), [faturaKesilenOdemeler]);
  const toplamKdvTutar      = useMemo(() => faturaKesilenOdemeler.reduce((s, o) => s + (o.kdvTutar || 0), 0), [faturaKesilenOdemeler]);
  const toplamKdvDahilTutar = useMemo(() => faturaKesilenOdemeler.reduce((s, o) => s + o.tutar, 0), [faturaKesilenOdemeler]);
  ...
  // ─── Fatura kes ──────────────────────────────────────────
  const faturaKes = async (odemeId, faturaNo, faturaTarihi, kdvOrani = 0, kdvTutar = 0, matrah = 0) => {
    // Önce fatura_kesildi + fatura_no + fatura_tarihi ile dene
    const { error } = await supabase.from('kira_odemeleri')
      .update({ fatura_kesildi: true, fatura_no: faturaNo || '', fatura_tarihi: faturaTarihi })
      .eq('id', odemeId);
    if (error) {
      console.warn('Fatura kes DB hatası:', error.message);
      // Kolon yoksa bile local state güncelle
    }
    // DB başarılı olsun ya da olmasın local state güncellenir
    setOdemeler(prev => prev.map(o => o.id === odemeId
      ? { ...o, faturaKesildi: true, faturaNo: faturaNo || '', faturaTarihi, kdvOrani, kdvTutar, matrah }
      : o));
  };
  ...
  const acGiderDuzenle = (g) => {
    setGiderForm({ mulkId: g.mulkId || '', tarih: g.tarih, tutar: g.tutar.toString(), kategori: g.kategori, aciklama: g.aciklama });
    setGiderDuzenle(g);
    setGiderModal(true);
  };
    ...
    <div>
      ...
      <div className="ozet-kartlar">
        {[
          ...
          { ikon:'📉', label:'Bu Ay Gider',              deger:`₺${buAyGider.toLocaleString('tr-TR')}`,                                                bg:'#FEE2E2', renk:'#DC2626' },
          { ikon:'💸', label:'Toplam Gider',             deger:`₺${toplamGider.toLocaleString('tr-TR')}`,                                              bg:'#FEF2F2', renk:'#B91C1C' },
          { ikon:'📊', label:'Net Kâr (Tahsil−Gider)',  deger:`₺${netKar.toLocaleString('tr-TR')}`,                                                   bg: netKar>=0?'#DCFCE7':'#FEE2E2', renk: netKar>=0?'#15803D':'#DC2626' },
        ].map(k => (
      ...
      <div className="tab-bar">
        ...
        <div className={`tab-item ${aktifTab==='giderler'?'aktif':''}`} onClick={()=>setAktifTab('giderler')}>
          📉 Giderler
          {giderler.length > 0 && (
            <span style={{ marginLeft:'6px', background:'#DC2626', color:'white', borderRadius:'10px', padding:'1px 7px', fontSize:'11px', fontWeight:'700' }}>
              {giderler.length}
            </span>
          )}
        </div>
        ...
        <div>
          ...
          <div className="panel">
            ...
            <div className="tablo-container">
              <table>
                <thead>
                  <tr><th>Mülk</th><th>Kiracı</th><th>TC No</th><th>Dönem</th><th>Matrah</th><th>KDV</th><th>KDV Dahil</th><th>Fatura No</th><th>Fatura Tarihi</th><th>Durum</th></tr>
                </thead>
                <tbody>
                  {faturaKesilenOdemeler.length === 0 ? (
                    ...
                    return (
                      <tr key={o.id} style={{ background:'linear-gradient(90deg,#F0FFF4,white)' }}>
                        ...
                        <td><div style={{ fontSize:'12px', color:'#64748B' }}>{m?.kiraci_tc || '—'}</div></td>
                        <td>{AY_ADLARI[dt.getMonth()]} {dt.getFullYear()}</td>
                        <td style={{ fontWeight:'800', color:'#059669' }}>₺{(o.matrah || o.tutar).toLocaleString('tr-TR')}</td>
                        <td style={{ fontWeight:'700', color:'#2563EB' }}>{kdvT > 0 ? `₺${kdvT.toLocaleString('tr-TR')}` : '—'}{o.kdvOrani > 0 && <span style={{ fontSize:'10px', color:'#94A3B8', marginLeft:'3px' }}>(%{o.kdvOrani})</span>}</td>
                        <td style={{ fontWeight:'800', color:'#4F46E5' }}>₺{o.tutar.toLocaleString('tr-TR')}</td>
                        ...
                        <td><span style={{ background:'#DCFCE7', color:'#14532D', padding:'4px 10px', borderRadius:'12px', fontSize:'12px', fontWeight:'700' }}>✅ Fatura Kesildi</span></td>
                      </tr>
                    );
                  })}
                </tbody>
      ...
      {aktifTab === 'giderler' && (
        <div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'12px', marginBottom:'16px' }}>
            {[
              { label:'Toplam Gider',       deger:`₺${toplamGider.toLocaleString('tr-TR')}`,       renk:'#DC2626', bg:'#FEE2E2', brd:'#FCA5A5' },
              { label:'Bu Ay Gider',        deger:`₺${buAyGider.toLocaleString('tr-TR')}`,         renk:'#D97706', bg:'#FEF3C7', brd:'#FDE68A' },
              { label:'Net Kâr (Tahsil−Gider)', deger:`₺${netKar.toLocaleString('tr-TR')}`,       renk: netKar>=0?'#15803D':'#DC2626', bg: netKar>=0?'#DCFCE7':'#FEE2E2', brd: netKar>=0?'#86EFAC':'#FCA5A5' },
            ].map(k => (
              <div key={k.label} style={{ background:k.bg, border:`1.5px solid ${k.brd}`, borderRadius:'12px', padding:'16px', textAlign:'center' }}>
                <div style={{ fontSize:'22px', fontWeight:'900', color:k.renk }}>{k.deger}</div>
                <div style={{ fontSize:'12px', color:k.renk, fontWeight:'600', marginTop:'4px' }}>{k.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="panel">
            <div className="panel-baslik">
              ...
              <button className="btn btn-primary btn-sm"
                onClick={() => { setGiderForm({ mulkId:'', tarih: new Date().toISOString().split('T')[0], tutar:'', kategori:'Bakım/Onarım', aciklama:'' }); setGiderDuzenle(null); setGiderModal(true); }}>
                + Gider Ekle
              </button>
            </div>
            <div className="tablo-container">
              <table>
                <thead>
                  <tr>
                    <th>Tarih</th><th>Kategori</th><th>Mülk</th><th>Tutar</th><th>Açıklama</th><th>İşlem</th>
                  </tr>
                </thead>
                <tbody>
                  {giderler.length === 0 ? (
                    ...
                    const m = mulkler.find(x => x.id === g.mulkId);
                    return (
                      <tr key={g.id} style={{ borderBottom:'1px solid #F1F5F9' }}>
                        <td style={{ fontWeight:'600', fontSize:'13px' }}>📅 {g.tarih}</td>
                        <td>
                          <span style={{ background:'#FEF3C7', color:'#92400E', padding:'3px 10px', borderRadius:'12px', fontSize:'12px', fontWeight:'700' }}>
                            {g.kategori}
                          </span>
                        </td>
                        <td>
                          {m
                            ? <span style={{ fontSize:'13px', fontWeight:'600' }}>{TIP_IKONLARI[m.tip]} {m.ad}</span>
                            : <span style={{ color:'#94A3B8', fontSize:'12px' }}>Genel Gider</span>}
                        </td>
                        <td>
                          <span style={{ fontWeight:'800', fontSize:'15px', color:'#DC2626' }}>
                            ₺{g.tutar.toLocaleString('tr-TR')}
                          </span>
                        </td>
                        <td style={{ fontSize:'13px', color:'#64748B', maxWidth:'200px' }}>{g.aciklama || '—'}</td>
                        <td>
                          <div style={{ display:'flex', gap:'4px' }}>
                            <button className="btn btn-secondary btn-sm" onClick={() => acGiderDuzenle(g)}>✏️</button>
                            <button className="btn btn-danger btn-sm" onClick={() => setGiderSilOnay(g)}>🗑️</button>
                          </div>
                        </td>
                      </tr>
                    );
            ...
            <>
              ...
              {detaySekme === 'sozlesme' && (
                <SozlesmeKarti mulk={gm} onYenile={(id,oran,yeniKira,sure) => {
                  ...
                  setDetayMulk(prev => ({ ...prev, aylikKira: yeniKira, yenilemeOrani: oran }));
                }} />
              )}
              ...
              {detaySekme === 'fatura_detay' && (() => {
                ...
                const bekleyen     = mulkFaturaOdemeleri.filter(o => (o.durum === 'odendi') && (!o.faturaKesildi));
                const faturaToplam = kesilmis.reduce((s, o) => s + (o.matrah || o.tutar), 0);
                const kdvToplam    = kesilmis.reduce((s, o) => s + (o.kdvTutar || 0), 0);
                const kdvDahilToplam = kesilmis.reduce((s, o) => s + o.tutar, 0);
                return (
                  <div>
                    {!gm.faturaKes && (
                    ...
                    {kesilmis.length > 0 && (
                      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'10px', marginBottom:'16px' }}>
                        {[
                          { label:'Matrah (KDV Hariç)', deger:`₺${faturaToplam.toLocaleString('tr-TR')}`,  renk:'#059669', bg:'#DCFCE7', brd:'#86EFAC' },
                          { label:'Toplam KDV',         deger: kdvToplam>0?`₺${kdvToplam.toLocaleString('tr-TR')}` :'₺0', renk:'#2563EB', bg:'#DBEAFE', brd:'#93C5FD' },
                          { label:'KDV Dahil Toplam',   deger:`₺${kdvDahilToplam.toLocaleString('tr-TR')}`,           renk:'#4F46E5', bg:'#EDE9FE', brd:'#C4B5FD' },
                        ].map(k => (
                          <div key={k.label} style={{ background:k.bg, border:`1px solid ${k.brd}`, borderRadius:'10px', padding:'12px', textAlign:'center' }}>
                            <div style={{ fontSize:'18px', fontWeight:'800', color:k.renk }}>{k.deger}</div>
                            <div style={{ fontSize:'11px', color:k.renk, fontWeight:'600', marginTop:'2px' }}>{k.label}</div>
                          </div>
                        ))}
                      </div>
                    ...
                    {kesilmis.length === 0 ? (
                      ...
                      return (
                        <div key={o.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', background:'#F0FFF4', border:'1px solid #86EFAC', borderRadius:'8px', padding:'12px 16px', marginBottom:'8px' }}>
                          <div>
                            <div style={{ fontWeight:'700', fontSize:'13px' }}>{AY_ADLARI[dt.getMonth()]} {dt.getFullYear()}</div>
                            <div style={{ fontSize:'12px', color:'#14532D' }}>Matrah: ₺{(o.matrah||o.tutar).toLocaleString('tr-TR')} {o.faturaNo && `| ${o.faturaNo}`}</div>
                            {o.kdvTutar > 0 && <div style={{ fontSize:'11px', color:'#2563EB' }}>KDV (%{o.kdvOrani}): ₺{o.kdvTutar.toLocaleString('tr-TR')} | KDV Dahil: ₺{o.tutar.toLocaleString('tr-TR')}</div>}
                            {o.faturaTarihi && <div style={{ fontSize:'11px', color:'#64748B' }}>Fatura Tarihi: {o.faturaTarihi}</div>}
                          </div>
                          <span style={{ background:'#DCFCE7', color:'#14532D', padding:'5px 12px', borderRadius:'12px', fontSize:'12px', fontWeight:'700' }}>✅ Fatura Kesildi</span>
                        </div>
                      );
                    })}
                  </div>
      ...
      <Modal acik={giderModal} kapat={() => { setGiderModal(false); setGiderDuzenle(null); }}
        baslik={giderDuzenle ? '✏️ Gider Düzenle' : '+ Yeni Gider Ekle'} genislik="500px">
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
          <Inp label="Tarih" name="tarih" tip="date" zorunlu form={giderForm} setForm={setGiderForm} />
          <Inp label="Tutar (₺)" name="tutar" tip="number" zorunlu form={giderForm} setForm={setGiderForm} />
          <Inp label="Kategori" name="kategori"
            options={GIDER_KATEGORILERI.map(k => ({ value:k, label:k }))}
            form={giderForm} setForm={setGiderForm} />
          <Inp label="Mülk (opsiyonel)" name="mulkId"
            options={mulkler.map(m => ({ value:m.id, label:`${TIP_IKONLARI[m.tip]||'🏠'} ${m.ad}` }))}
            form={giderForm} setForm={setGiderForm} />
          <Inp label="Açıklama" name="aciklama" tam form={giderForm} setForm={setGiderForm} />
        </div>
        <div style={{ display:'flex', gap:'8px', marginTop:'20px', justifyContent:'flex-end' }}>
          <button className="btn btn-secondary" onClick={() => { setGiderModal(false); setGiderDuzenle(null); }}>İptal</button>
          <button className="btn btn-primary" onClick={giderKaydet} disabled={giderKaydediyor}>
            {giderKaydediyor ? '⏳ Kaydediliyor...' : giderDuzenle ? '💾 Güncelle' : '✅ Kaydet'}
          </button>
        </div>
      </Modal>

      {/* ═══ Gider Sil Onay Modal ═══ */}
      <Modal acik={!!giderSilOnay} kapat={() => setGiderSilOnay(null)} baslik="🗑️ Gider Sil" genislik="380px">
        {giderSilOnay && (
          <>
            <div style={{ textAlign:'center', padding:'8px 0 20px' }}>
              <div style={{ fontSize:'48px', marginBottom:'10px' }}>⚠️</div>
              <p style={{ fontSize:'15px', color:'#374151', fontWeight:'600' }}>
                <b>₺{giderSilOnay.tutar.toLocaleString('tr-TR')}</b> tutarındaki <b>{giderSilOnay.kategori}</b> giderini silmek istiyor musunuz?
              </p>
              <p style={{ fontSize:'12px', color:'#EF4444', marginTop:'6px' }}>Bu işlem geri alınamaz!</p>
            </div>
            <div style={{ display:'flex', gap:'10px' }}>
              <button className="btn btn-secondary" style={{ flex:1 }} onClick={() => setGiderSilOnay(null)}>Vazgeç</button>
              <button className="btn btn-danger" style={{ flex:1 }} onClick={() => giderSil(giderSilOnay.id)}>🗑️ Evet, Sil</button>
            </div>
          </>
        )}
      </Modal>
